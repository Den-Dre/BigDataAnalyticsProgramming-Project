...
for file_name in file_names:
    ...
    plt.plot(df.index[::step], df.iloc[::step], linewidth=1, label=r'$n_{min} = $ ' + file_name.split('/')[-1].split('.out')[0][4:])
plt.xlabel('Number of examples trained with')
plt.ylabel('Accuracy')
plt.title(r"VFDT's accuracy in function of $n_{min}$")
order = [1, 0, 2, 3, 4]
handles, labels = plt.gca().get_legend_handles_labels()
leg = plt.legend(handles=[handles[i] for i in order], labels=[labels[i] for i in order])
